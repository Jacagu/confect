...
__export(schema_exports, {
  confectSystemSchema: () => confectSystemSchema,
  confectSystemSchemaDefinition: () => confectSystemSchemaDefinition,
  confectTableSchemas: () => confectTableSchemas,
...
var Id = (tableName2) => import_effect.Schema.String.pipe(
  import_effect.Schema.annotations({ [ConvexId]: tableName2 })
);
...
var SystemFields = (tableName2) => import_effect3.Schema.Struct({
  _id: Id(tableName2),
  _creationTime: import_effect3.Schema.Number
});
var extendWithSystemFields = (tableName2, schema) => import_effect3.Schema.extend(schema, SystemFields(tableName2));

// src/server/schema.ts
var confectTableSchemas = {
  _scheduled_functions: import_effect4.Schema.Struct({
    name: import_effect4.Schema.String,
    args: import_effect4.Schema.Array(import_effect4.Schema.Any),
    scheduledTime: import_effect4.Schema.Number,
    completedTime: import_effect4.Schema.optional(import_effect4.Schema.Number),
    state: import_effect4.Schema.Union(
      import_effect4.Schema.Struct({ kind: import_effect4.Schema.Literal("pending") }),
      import_effect4.Schema.Struct({ kind: import_effect4.Schema.Literal("inProgress") }),
      import_effect4.Schema.Struct({ kind: import_effect4.Schema.Literal("success") }),
      import_effect4.Schema.Struct({
        kind: import_effect4.Schema.Literal("failed"),
        error: import_effect4.Schema.String
      }),
      import_effect4.Schema.Struct({ kind: import_effect4.Schema.Literal("canceled") })
    )
  }),
  _storage: import_effect4.Schema.Struct({
    sha256: import_effect4.Schema.String,
    size: import_effect4.Schema.Number,
    contentType: import_effect4.Schema.optional(import_effect4.Schema.String)
  })
};
var tableSchemasFromConfectSchema = (confectSchema) => ({
  ...import_effect4.Record.map(confectSchema, ({ tableSchema }, tableName2) => ({
    withSystemFields: extendWithSystemFields(tableName2, tableSchema),
    withoutSystemFields: tableSchema
  })),
  ...import_effect4.Record.map(confectTableSchemas, (tableSchema, tableName2) => ({
    withSystemFields: extendWithSystemFields(tableName2, tableSchema),
    withoutSystemFields: tableSchema
  }))
});
var ConfectSchemaDefinitionImpl = class {
  ...
  constructor(confectSchema) {
    ...
    this.convexSchemaDefinition = (0, import_effect4.pipe)(
      confectSchema,
      import_effect4.Record.map(({ tableDefinition }) => tableDefinition),
